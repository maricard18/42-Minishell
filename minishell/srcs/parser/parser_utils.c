...
int count_and_or_tokens(t_token *command_token_list)//count the number of AND and OR
{
	...
	while (current_token)
	{
		if (current_token->type == TOKEN_AND
			...
			i++;
		else if (current_token->type == TOKEN_OPEN_PAR)
...
void handle_paranthesis(t_token **tokens_list, t_parsed **command)
{
	...
	{
		if ((*tokens_list)->next->type == TOKEN_OPEN_PAR)
			i++;
		else if ((*tokens_list)->next->type == TOKEN_CLOSE_PAR)
			i--;
...
void skip_to_matching_parenthesis(t_token **command_token_list)
{
	...
	{
		if ((*command_token_list)->type == TOKEN_OPEN_PAR)//if the token is open parenthesis
			i++;
		else if ((*command_token_list)->type == TOKEN_CLOSE_PAR)//if the token is close parenthesis
			i--;
